...
public class TrackerPanel extends VideoPanel implements Scrollable {
  ...
  protected boolean isKeyOneDown, isKeyTwoDown, isKeyThreeDown, isKeyFourDown, isKeyFiveDown, isKeySixDown, isKeySevenDown, isKeyEightDown, isKeyNineDown, isKeyTenDown;
...
//    }
  ...
  public void setSelectedPoint(TPoint point) {
    ...
    if (point == null) {
      ...
      Step step = null;
      //setSelectedPoint(null);
      Iterator<TTrack> it = getUserTracks().iterator();
      while(it.hasNext()) {
        track = it.next();
        step = track.getStep(point, this);
        if (step != null) break;
...
//				new Thread(runner).start();
    ...
    addKeyListener(new KeyAdapter() {
      public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_SHIFT) {
        	keyPressHandle(isShiftKeyDown, e);
        }
   ...
   */
  protected void keyPressHandle(boolean isKeyDown, KeyEvent e) {
	  if (!isKeyDown) {
  		isKeyDown = true;
  ...
  protected void handleKeyPress(KeyEvent e) {
    ...
    double delta = (e.isShiftDown())? 10: 1; // add other bools to this maybe
    double dx = 0, dy = 0;
    switch(e.getKeyCode()) {
	    case KeyEvent.VK_UP:
	      dy = -delta;
  ...
  public void propertyChange(PropertyChangeEvent e) {
    ...
    if (name.equals("step") || name.equals("steps")) { // from tracks/steps //$NON-NLS-1$ //$NON-NLS-2$
      ...
      boolean invertCursor = (isShiftKeyDown || isKeyOneDown || isKeyTwoDown || isKeyThreeDown || isKeyFourDown);
