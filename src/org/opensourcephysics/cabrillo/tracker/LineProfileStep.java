...
public class LineProfileStep extends Step {
  ...
  protected double xAxisToLineAngle, lineAngle;
  ...
  */
  ...
  class LineEnd extends TPoint {
    ...
    public void setXY(double x, double y) {
      ...
      // find the mouse displacement (shared hypotenuse) 
      double dx = x-getX();
      double dy = y-getY();
      ...
      if (line.trackerPanel != null) {
        scaleX = line.trackerPanel.getCoords().getScaleX(n);
        scaleY = line.trackerPanel.getCoords().getScaleY(n);
        axisTiltAngle = line.trackerPanel.getCoords().getAngle(n);
      }
      
      double dx1 = (getLineEnd0().getX() - getLineEnd1().getX()) / scaleX;
      double dy1 = (getLineEnd0().getY() - getLineEnd1().getY()) / scaleY;
      lineAngle = Math.atan2(dy1, dx1);
      xAxisToLineAngle = axisTiltAngle - lineAngle;
      //if (Double.isNaN(xAxisToLineAngle)) {
      	//xAxisToLineAngle = 0;
      //}
      line.angleField.setValue(xAxisToLineAngle);
